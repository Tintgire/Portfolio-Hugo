...
- **Earth 3D** — modèle GLTF rendu via React Three Fiber, `OrbitControls` contraints à la rotation horizontale, `autoRotate`, avec rim violet et fill rose en directional lights pour le vibe cosmique
- Formulaire de contact via **EmailJS** — l'arpège `playSubmit` se déclenche au succès

### Cross-cutting

- **`prefers-reduced-motion`** — Lenis bypass, particules sans rotation, hover magnétique désactivé, son force-disabled
- **Mobile-aware** — hook `useIsMobile` qui ajuste le nombre de particules et simplifie les layouts
- **Pattern portail** — drawers et modal son portail vers `document.body` pour échapper aux stacking contexts créés par les section wrappers
